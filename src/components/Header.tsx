      ...
      <div className="web-border-space">
        <div id="header-wrapper">
          ...
          <nav className={toggleMenu ? "active" : ""}>
            <ul>
              <li><NavLinkCustom to="/" clickEvent={closeMenu}>Home</NavLinkCustom></li>
              <li>
                ...
                <div className="nav-lv2">
                  <ul>
                    {projectList.map((project:Project) => (
                      <li key={project.id}>
                          <NavLinkCustom to={`/projects/${project.id}`} clickEvent={closeMenu}>{project.title}</NavLinkCustom>
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="nav-lv2">
                  <ul>
                    {homeworkList.map((homework:Homework) => (
                      <li key={homework.id}>
                          <NavLinkCustom to={`/homeworks/${homework.id}`} clickEvent={closeMenu}>{homework.title}</NavLinkCustom>
                      </li>
                    ))}
                  </ul>
                </div>
              </li>
              <li><NavLinkCustom to="/tasks" clickEvent={closeMenu}>Task Manager</NavLinkCustom></li>
            </ul>
          </nav>
